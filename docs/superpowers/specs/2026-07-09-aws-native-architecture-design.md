...
- **Existing assets to reuse:** `apps/dashboard-django` (Django + React/Vite, has a `Dockerfile`), `apps/etl-python` (completed port of the former `apps/etl-clojure`, per [[etl-python-migration]], has a `Dockerfile`), the existing Postgres schema, and the existing GitHub Actions CI (`tests.yml`, `docker.yml`).
...
1. **Hybrid pragmatic (chosen):** **ECS Express Mode** for the dashboard container as-is, Fargate one-off tasks for ETL, Aurora Serverless v2 for the database. Lowest porting effort (reuses the existing Dockerfile nearly unmodified) and it's AWS's own recommended replacement now that App Runner is closed to new customers. Trade-off versus the original App Runner plan: Express Mode always runs behind an Application Load Balancer and needs a minimum of 1 task always on (§4), so the dashboard tier carries a ~US$25–30/month fixed cost even when idle (§9) — it is no longer a scale-to-zero tier, just the lowest-effort managed-container path AWS still offers.
...
|---|---|---|
| `dashboard` service (Django + React) | **ECS Express Mode** (Fargate + an automated ALB) | Runs the existing container image with minimal changes; Express Mode provisions the ALB (with SSL/TLS on its default URL), networking, and autoscaling for it. Runs with a **minimum of 1 task always on**: an ALB cannot wake a scaled-to-zero service on an incoming request — with 0 healthy targets it just returns 503s, and ECS autoscaling reacts to metrics, not to a held request the way App Runner did — so the dashboard tier has no scale-to-zero (cost impact in §9). |
...
ECS Express Mode (Django) ──generates a presigned S3 PUT URL──▶ Browser
   ...
   │                                                          │ downloads object from S3, loads via psycopg2
   ▼                                                          ▼
Aurora Serverless v2 (Postgres) ◀────────────────────────────┘
```

Key decisions:
- **The large file never transits through the dashboard container/Django.** Django's only job in the upload path is issuing a presigned URL; the browser uploads straight to S3. This avoids the ALB's/Fargate task's request-size and timeout limits and keeps the app container stateless and small. Known v1 limitation: a single presigned PUT has no resume support — an interrupted 1.5GB upload restarts from scratch (multipart presigned uploads would fix this at the cost of a considerably more complex signing endpoint).
- **The ETL reads its input by downloading from S3, not streaming.** The ported CLI takes a local `--file` path; the containerized task gains a small entry step that downloads the S3 object to Fargate ephemeral storage (20GB by default — ample for a 1.5GB export) and then runs the existing pipeline unchanged. Download-then-run is deliberate: the two-pass design (count pass for the progress-bar total, then the load pass) would otherwise read the object from S3 twice over the network. This is an app change, listed in §10.
...
- **Step Functions**, not a bare EventBridge→Lambda→RunTask call, because it gives free retry/failure handling around the Fargate task (distinct from the ETL's own status writes above) without hand-rolling that logic.
- **Single-flight is enforced in the ETL, not in Step Functions.** Step Functions Standard has no native per-state-machine concurrency cap — each EventBridge event starts its own execution, so "cap executions at 1" is not directly expressible in Terraform. Instead, the ETL takes a Postgres advisory lock at startup and exits immediately (recording a `skipped` `etl_run` row) if another import already holds it. Two overlapping uploads therefore can't load into the same tables simultaneously, with no extra infrastructure.
- **During an import, the dashboard shows empty/partial data.** The v1 truncate-and-reload strategy empties all tables before reloading, so a dashboard opened mid-import sees an incomplete dataset until the run finishes. Expected v1 behavior for a single-user tool, not a bug — the `etl_run` status row lets the dashboard say "import in progress" if desired.
...
- The endpoint that mints presigned URLs requires a static shared-secret header (a token only the owner has — simplest possible gate that doesn't require standing up `django.contrib.auth`). The token is entered by the owner at upload time (e.g. a password field in the upload UI) — it must never be embedded in the frontend JS bundle, which is publicly downloadable by anyone with the URL.
...
- An AWS Budgets alarm (email/SNS) at a monthly threshold of ~US$50 — above the expected ~US$35–45 baseline (§9) so it fires only on anomalies, not every month — acts as a cost tripwire in case the above is ever bypassed.
...
- **Open item for the Terraform follow-on plan:** ECS Express Mode is a very new feature (~December 2025) and automates a chunk of this networking (ALB, target groups, autoscaling) itself. Confirmed from the Express Mode docs: the default endpoint includes SSL/TLS, so no custom domain is needed for HTTPS. Still open: exactly what subnet placement and egress path it defaults to for the Fargate task — if its automation insists on private subnets for the task, that reintroduces the NAT-vs-VPC-endpoints cost trade-off from the ETL task, and the two should be resolved the same way for consistency.
...
- **Aurora auto-pause requires zero open connections.** With the dashboard task now always-on (§4), Django must not hold idle connections or Aurora will never pause and bills its minimum ACU 24/7: set `CONN_MAX_AGE=0` (or a short value), and the health-check endpoint (below) must not touch the database. The first request after a pause also tolerates the ~15-second resume — acceptable for personal use, but Django's DB connection timeout must be set above it.
- **ALB health checks vs `ALLOWED_HOSTS`:** the ALB's health check sends the task's private IP as the `Host` header, which a strict `ALLOWED_HOSTS` rejects with a 400 — marking targets permanently unhealthy and making ECS kill and replace tasks in a loop, so the service never stabilizes. The app therefore needs a `/healthz` endpoint that is exempt from host validation (and, per the auto-pause bullet above, does not query the database), with the ALB health check pointed at it. Blocking for go-live, same tier as the §8 production-config items.
...
- **ECS Express Mode**: the dashboard runs a minimum of 1 Fargate task 24/7 (~US$9/month at 0.25 vCPU / 0.5GB) because an ALB cannot wake a scaled-to-zero service on request (§4), plus **the ALB is always-on and bills a fixed ~US$16–20/month regardless of traffic**. Together these are the direct cost of App Runner no longer being available, and they dominate the bill.
- **Public IPv4 addresses**: ~US$3.65/month each since AWS started charging for IPv4 (2024) — the ALB uses one per AZ (2) and the always-on dashboard task uses one, adding ~US$11/month that's easy to miss in a "public subnets, no NAT" design; the ETL task's IP only bills for the minutes a run is active.
...
Expect a bill in the ~US$35–45/month range: ALB (~US$16–20) + minimum dashboard task (~US$9) + public IPv4 (~US$11) + Aurora storage/compute baseline, not data transfer or request volume. This is meaningfully higher than the original App Runner-based estimate (low double digits) — the compounded consequence of App Runner's discontinuation forcing an ALB, a minimum task, and their IPv4 addresses into the design. If cost is a hard constraint rather than a rough budget, revisit Option 2 (Lambda-first, §3), which eliminates all three of those line items.
...
- Changes to the ETL/dashboard application code beyond what's needed to run in these services — in scope as app changes: the presigned-URL upload endpoint, the `etl_run` status table, the ETL's S3-download entry step and advisory lock (§5.1), and the `/healthz` endpoint (§6). Functional behavior of the pipeline itself is unchanged from [[etl-python-migration]].
